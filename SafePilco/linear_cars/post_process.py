...
def check_for_collisions(X, bounds):
    collision = False
    for i in range(len(X)):
        if X[i,0] > bounds[0,0] and X[i,0] < bounds[0,1] and X[i,2] > bounds[0,1] and X[i,2] < bounds[1,1]:
            collision = True
    return collision
...
runs_so_far = 10  # number of random seeds run (numbered 0 to runs_so_far)
T = 25
J = 5
...
path = 'results/'
name = ''
for i in range(runs_so_far):
    X.append(np.loadtxt(path + 'X_' + name  + str(i) + '.csv', delimiter=','))
    X_eval.append(np.loadtxt(path + 'X_eval_' + name  + str(i) + '.csv', delimiter=','))
    all_returns_sampled.append(np.loadtxt(path + 'evaluation_returns_sampled_'+ name  + str(i) + '.csv', delimiter=','))
    all_returns_full.append(np.loadtxt(path + 'evaluation_returns_full_' + name  + str(i) + '.csv', delimiter=','))
...
for run in range(runs_so_far):
    ...
    for j in starting_indices:
        collision_array.append(check_for_collisions(X[run][j:j+T, :], run_bounds))
